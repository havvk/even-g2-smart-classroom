...
| `0x05-20` | Translation | 独立双语对照翻译服务 (顶部视口、逐句对照) (详见第 30 章) |
...
| `0x0B-20` | Conversate | 实时语音同传听写与双语字幕 (`proto_translate_ext`，详见第 29 章) |
...
**关键洞察**：
- **首次冷启动时**（`bt.pklg` / `bt3.pklg`，包含 Auth+Setup）：固件首次初始化，官方 APP 统一填入 `field_4=59`, `field_5=585` 作为默认视口/最大画布规格。
- **后续热重推时**（`multiprompts.pklg`，Session 已建立，仅做内容重推）：官方 APP 动态将 `field_4` 设为**实际总页数** (2)，`field_5` 设为**实际总行数** (13)——精确匹配 2 页内容 (Page 0=10 行 + Page 1=3 行)。
- **规范设计**：在冷启动（未完成 Auth/Setup）时须保留 `59`/`585` 完成画布规格挂载；在热重推（已 Auth）时，改发实际内容量（页数/行数），帮助固件预知边界。
...
## 29. Even G2 对话模式 (Conversate / Translate) 全协议解密与底部三行流式滚动视口规范 (基于 2026-09 真实抓包) 🆕

### 29.1 业务背景与用户实测抓包实证
基于使用 Even Realities 官方 App 配合 G2 眼镜捕获的真实蓝牙通信日志：
- `tests/对话模式_无AI提示.pklg`（186 KB，含 84 个 0xAA 报文）
- `tests/__pycache__/对话模式_无AI提示_有退出对话模式.pklg`（73 KB，含 39 个 0xAA 报文）

通过逆向解析工具 `scripts/deep_decode_proto.py`，100% 还原了现场录制的真实会话文字序列：
- Pkt #015: `"大家好。"`
- Pkt #022: `"为什么在最下面呢？"`
- Pkt #036: `"哎，为什么上面是空的呢？"`
- Pkt #057: `"喂喂喂，喂喂喂，下面只有 3 行吗？"`
- Pkt #071: `"好像只有 3 行。"`
- Pkt #079: `"嗯，原来如此。"`
- 在退出用例抓包中录入 `"今天是一个很好的天气。"`，并在 Pkt #036 精确捕获了官方退出对话模式的**终止注销报文**。

---

### 29.2 物理视口机制：底部三行吸底滚动与上半区天然透明
现场直观观察证实：“对话模式在底部三行区域转写内容，自动向上滚动，上半部分没有内容变动”。
底层固件与光学机制彻底揭秘如下：
1. **固件视口吸底硬编码（Bottom-Anchored Viewport）**：
   - 官方固件窗口管理器（Window Manager）为 `Service 0x0B-20 Type 6` 转写流分配的原生可视区域，**被固定限制在 HUD 最底部的 3 行空间**；
   - 字符持续增量打字，单句定稿加标点（`is_final=1`）后平滑向上滚一行，历史行向上顶出并销毁，**始终保持最多 3 行可见**；
2. **上半区天然透明（MicroLED 物理黑电平）**：
   - MicroLED 是像素自发光芯片。在未触发 AI 提示卡片时，由于上半区没有任何字符点阵被激活，驱动电流为 0，点阵完全不发光，形成物理级全透光（全黑透明）；
   - 这不仅不会遮挡佩戴者的真实视野，更为自研系统在上半区叠加 AI 话题延展与意图解读提供了完全干净的独立显存视口。

---

### 29.3 通信链路与 GATT 读写通道映射
| 方向 | 物理 GATT Characteristic | 属性 | Service ID | 作用 |
| :--- | :--- | :--- | :--- | :--- |
| **手机 $\rightarrow$ 眼镜** | `00002760-08c2-11e1-9073-0e8ac72e5401` | Write Without Response | `0x0B-20` (`svc_hi=0x0B, svc_lo=0x20`) | 下发会话初始化、流式字幕文本与退出指令 |
| **眼镜 $\rightarrow$ 手机** | `00002760-08c2-11e1-9073-0e8ac72e5402` | Notify | `0x0B-00` (`svc_hi=0x0B, svc_lo=0x00`) | 固件回发接收确认 ACK（固定 9 字节） |

#### 固件 ACK 回执格式 (固定 9 字节)：
```text
AA 12 [Seq] 09 01 01 0B 00 08 A2 01 10 [MsgId] 52 00 [CRC16]
```
- `Tag 1 (varint) = 162 (0xA2)`：ACK 成功标记；
- `Tag 2 (varint) = MsgId`：回执确认对应的手机端 MsgId；
- `Tag 10 (bytes) = <0B>`：状态码 0。

---

### 29.4 Protobuf 报文全量 Schema 定义 (`Service 0x0B-20`)

```protobuf
syntax = "proto3";
package even.g2;

// Service 0x0B-20 主结构体
message ConversatePacket {
  uint32 type = 1;                     // 核心命令类型: 1 = 会话控制, 5 = AI胶囊提示, 6 = 流式转写, 255 = 提交标杆
  uint32 msg_id = 2;                  // 会话内单调递增 ID (必须与帧头 seq 一致)
  
  // 当 type = 1 时有效 (会话启停控制)
  SessionControl control = 3;         // Tag 3 (Field 3)
  
  // 当 type = 5 时有效 (上半区 AI 胶囊提示卡片) 🆕
  AIPromptPayload prompt = 7;         // Tag 7 (Field 7, Wire Tag 0x3A)
  
  // 当 type = 6 时有效 (流式转写负载)
  TranscriptPayload transcript = 8;   // Tag 8 (Field 8)
  
  // 当 type = 255 时有效 (提交/心跳标记)
  MarkerPayload marker = 11;          // Tag 11 (Field 11, 空内容 5A 00)
}

message AIPromptPayload {
  uint32 card_type = 1;               // 胶囊卡片类型: 4 = 标准圆角胶囊边框 (带智能 Emoji 图标)
  string title = 2;                   // 胶囊主标题文本 (显示在圆角边框内，如 "奥威尔1949年出版的作品是《1984》")
  string detail = 3;                  // 详细解读内容 (镜腿滑动交互时展开查看完整长文本)
  uint32 status = 4;                  // 状态标识 (0)
}

message SessionControl {
  uint32 action = 1;                  // 1 = 会话启动初始化 (Start), 2 = 会话停止退出 (Stop/Exit)
  bytes config_flags = 2;             // 语言/视口控制标志
  uint32 status = 4;                  // 状态位 (退出时为 0)
  repeated LanguageItem languages = 5;// 支持的语言列表 (OFF, AUTO, ZH, TW, JA, ES, FR, DE, IT, KO)
  string default_target = 6;          // 默认目标语言 (如 "OFF")
}

message LanguageItem {
  string code = 1;                    // 如 "ZH", "EN", "AUTO", "OFF"
  string display_name = 2;            // 如 "中文（简体）", "自动检测", "关闭"
}

message TranscriptPayload {
  string text = 1;                    // UTF-8 编码的语音转写文本
  uint32 is_final = 2;                // 核心状态位: 0 = 流式动态识别打字中; 1 = 终态断句定稿 (触发向上平滑滚行)
}

message MarkerPayload {
  // 空负载，仅作为显存 Flush 同步脉冲
}
```

---

### 29.5 真实十六进制数据包剖析 (Raw Hex & Wire Format)

#### 1. 会话初始化启动报文 (`Type 1 Action 1`, 长度 227B)
```text
AA 21 3B E3 01 01 0B 20 08 01 10 3B 1A DA 01 08 01 12 0A 08 01 10 01 18 00 20 01 28 00 20 00 ... [CRC16]
```
- `08 01`：`type = 1`；
- `10 3B`：`msg_id = 59`；
- `1A DA 01`：`Field 3` 长度 218 字节，包含支持的所有双向语言列表。

#### 2. 流式转写打字中报文 (`Type 6 is_final=0`, 增量刷新)
```text
AA 21 3C 0F 01 01 0B 20 08 06 10 3C 42 07 0A 03 E5 96 82 10 00 [AC FB]
```
- `08 06`：`type = 6`；
- `10 3C`：`msg_id = 60`；
- `42 07`：`Field 8` 长度 7 字节：
  - `0A 03 E5 96 82`：`Tag 1` 长度 3 字节，UTF-8 内容 `"喂"`；
  - `10 00`：`Tag 2 = 0` (`is_final = 0`)，指示光机在最底行原位打字显示，不滚行。

#### 3. 终态断句加标点报文 (`Type 6 is_final=1`, 触发平滑滚屏)
```text
AA 21 41 30 01 01 0B 20 08 06 10 41 42 28 0A 24 E5 96 82 E5 96 82 ... E3 80 82 10 01 [CRC16]
```
- `42 28`：`Field 8` 长度 40 字节；
- `0A 24 ...`：UTF-8 文本 `"喂喂喂，大家好，大家好。"`（带句号）；
- `10 01`：`Tag 2 = 1` (`is_final = 1`)，指示光机锁定本句为历史记录，将当前 3 行视口平滑向上滚移一行。

#### 4. 显存锁存提交标杆帧 (`Type 255`, 长度 9B)
```text
AA 21 3F 09 01 01 0B 20 08 FF 01 10 3F 5A 00 [CA 8F]
```
- `08 FF 01`：`type = 255`；
- `10 3F`：`msg_id = 63`；
- `5A 00`：`Tag 11` 长度 0，触发显存锁存提交，防止字迹渐变丢失。

#### 5. 退出对话模式黄金注销帧 (`Type 1 Action 2`, 长度 12B)
```text
AA 21 4A 0C 01 01 0B 20 08 01 10 4A 1A 04 08 02 20 00 [82 AB]
```
- `08 01`：`type = 1`；
- `10 4A`：`msg_id = 74`；
- `1A 04`：`Field 3` 长度 4 字节：
  - `08 02`：`action = 2` (**退出/注销会话**)；
  - `20 00`：`status = 0`；
- **效果**：眼镜光机接收后安全注销 `0x0B-20` 视口，光机恢复 Dashboard 待机桌面，杜绝黑屏假死。

---

### 29.6 自研网关工程落地对照清单 (`mobile_gateway_ios`)

| 功能函数 | 推荐实现位置 | 报文构造逻辑与 Wire Format |
| :--- | :--- | :--- |
| **开启对话会话** | `G2ProtocolEncoder.buildConversateInitPacket` | 构造 `0x0B-20 Type 1 Action 1`，下发初始语言列表配置（225 字节）； |
| **下发流式打字** | `G2ProtocolEncoder.buildConversateTranscriptPacket` | 构造 `0x0B-20 Type 6`，入参 `text: String, isFinal: false`，编码为 `Tag 1 text` 与 `Tag 2 is_final=0`；在屏幕最底行原地打字实时刷新； |
| **下发定稿断句** | `G2ProtocolEncoder.buildConversateFinalizePacket` | 构造 `0x0B-20 Type 6`，入参 `text: String, isFinal: true`，编码为 `Tag 1 text` 与 `Tag 2 is_final=1`；约 0.5s 后补齐句尾标点并锁入历史； |
| **显存锁存提交** | `G2ProtocolEncoder.buildConversateSyncPacket` | 构造 `0x0B-20 Type 255`，`Tag 11` 空内容（`5A 00`），触发物理显存强制刷写； |
| **优雅注销退出** | `G2ProtocolEncoder.buildConversateExitPacket` | 构造 `0x0B-20 Type 1 Action 2`（`1A 04 08 02 20 00`），释放显存并安全返回系统待机桌面，杜绝假死。 |

---

### 29.7 物理硬件实测验证与避坑指南 (2026-09-12 真机 100% 验证闭环) 🏆

在真实 Even G2 智能眼镜硬件（`Even G2_32_L`，固件版本 `2.2.10.10`）上，通过自动化 Python 脚本（`scripts/test_conversate_bottom_display.py`）完成了对 `0x0B-20` 对话模式从握手、避让、流式打字、定稿断句到滚行换行的全链路物理上屏验证。

#### 1. 真实硬件呈现效果记录
- **流式打字阶段**：下发 `is_final=0` 的文本 `"你好，智能眼镜"`，屏幕底部第三行以极低延迟实时原位打字渲染；
- **定稿断句阶段**：下发 `is_final=1` 的文本 `"你好，智能眼镜！"`，文字停顿约 0.5 秒后平滑补齐句末标点 `！`，并将该句锁定为已定稿行；
- **自动滚行阶段**：下发第二句定稿文本 `"第二行测试：实时转写顺利上屏了吗？"`，固件窗口管理器自动将第一句向上推移一行，并在新空出的底部行打出第二句文字，下半部始终保持最多三行滚动可见，视觉呈现与官方 App 毫无二致。

#### 2. 核心工程避坑指南 (CRITICAL PITFALLS)

> [!CAUTION]
> **陷阱一：固件开场动画 2.10 秒显存保护期（吞包机制与二分法高精度测定）**
> - **底层机制**：当眼镜接收到 `Type 1 Action 1`（启动对话）报文后，固件窗口管理器接管 MicroLED 显存，播放系统级全屏模态 Toast 动画（“启动对话”）。在此期间，固件显存驱动处于排他保护锁定状态，**任何提前下发的文字报文或 Sync 锁存报文均会被固件静默丢弃**；
> - **二分法高精度实测收敛全记录 (精度 50ms)**：
>   采用专门测定脚本 [`scripts/binary_search_cooldown.py`](file:///Users/l.ylive.cn/OneDrive/smart-glass/scripts/binary_search_cooldown.py) 在真机上逐轮收敛测定：
> 
>   | 轮次 | 冷却等待时间 | 镜片物理呈现现象 | 结果判定 |
>   | :--- | :--- | :--- | :--- |
>   | 基线 A | 0.50 秒 | 进出对话正常，中间完全空白（被动画独占吞没） | ❌ 失败 |
>   | 基线 B | 4.50 秒 | 原位流式打字，句尾补齐感叹号，平滑自动滚行 | ✅ 成功 |
>   | 第 1 轮 | 2.50 秒 | 原位流式打字与定稿断句完全正常 | ✅ 成功 |
>   | 第 2 轮 | 1.50 秒 | 进出对话正常，中间完全空白（被吞） | ❌ 失败 |
>   | 第 3 轮 | 2.00 秒 | 进出对话正常，中间显示为空 | ❌ 失败 |
>   | 第 4 轮 | 2.25 秒 | 进出对话与中间文字显示正常 | ✅ 成功 |
>   | 第 5 轮 | 2.10 秒 | 进出对话与中间文字显示正常 | ✅ 成功 |
>   | 第 6 轮 | 2.05 秒 | 进出对话正常，中间显示空白（被吞） | ❌ 失败 |
> 
> - **测定结论与工程参数**：
>   1. **硬件物理死界线**：$$\mathbf{2.05\text{s} < T_{\text{cooldown}} \le 2.10\text{s}}$$
>      低于 2.10 秒（如 2.05 秒），文字必被固件吞没；$\ge 2.10$ 秒显存驱动即刻解禁。
>   2. **自研网关最优推荐值**：推荐设置为 **`2.20 ~ 2.30 秒`**（仅需比物理界限预留 100~200ms 的 BLE 抖动余量）。
>   3. **优于官方 App**：官方 App 采用了极其保守的 **3.34 秒** 等待；自研网关采用 **2.25 秒** 策略，可比官方 App **提前整整 1 秒出字**，极大提升佩戴者的实时交互体感！
>   4. **退出会话清理时间**：下发 `build_conversate_exit` 后，网关应维持 BLE 连接至少 **1.5 ~ 2.0 秒** 供固件注销会话与退回待机桌面，避免会话悬挂。

> [!WARNING]
> **陷阱二：全局 Sequence 与 MsgId 必须单调递增，严禁回退重置**
> - **底层机制**：G2 蓝牙通信协议栈维护着严格的滑动窗口与包序号检查。7 步 Auth 握手已固定占用了 `seq = 1 ~ 7`；
> - **致命错误**：如果在进入业务模式后将 `seq` 重置为 1，或者让 `msg_id` 与 `seq` 脱节错位，眼镜协议栈会将这些报文判定为“历史重复帧（Duplicate Retransmission）”而拒绝解析处理；
> - **工程铁律**：`seq` 必须在同一个 BLE 会话生命周期内全局自增（`0x01 ~ 0xFF` 循环溢出），且 `ConversatePacket.msg_id` 必须严格与外层帧头 `seq` 保持一致（即 `msg_id = seq`）。

> [!TIP]
> **陷阱三：避免多模式瞬时并发或退出弹窗覆盖**
> - `0x05-20`（翻译模式）退出时会触发固件全屏弹窗 **“翻译已保存”**，该弹窗会强制覆写前台视口达 3 秒；
> - 在未完成当前视口验证前，切勿交替触发另一模式的退出流程，以免退出弹窗冲刷冲掉刚渲染好的文字内容。

#### 3. 极简实测验证代码参考
完整经过硬件实测验证的单项测试工具已沉淀于：
👉 [`scripts/test_conversate_bottom_display.py`](file:///Users/l.ylive.cn/OneDrive/smart-glass/scripts/test_conversate_bottom_display.py)
```python
# 核心时序流程：
await client.write_gatt_char(CHAR_WRITE, build_conversate_init(seq=8), response=False)
await asyncio.sleep(4.5)  # 严格避开开场动画独占期！
await client.write_gatt_char(CHAR_WRITE, build_conversate_transcript(seq=9, text="你好，智能眼镜", is_final=False), response=False)
await asyncio.sleep(1.0)
await client.write_gatt_char(CHAR_WRITE, build_conversate_transcript(seq=10, text="你好，智能眼镜！", is_final=True), response=False)
await client.write_gatt_char(CHAR_WRITE, build_conversate_sync(seq=11), response=False)
```

### 29.8 官方原生 AI 胶囊提示全协议解密与双视口生命周期机制 (基于 `tests/对话模式_有AI提示.pklg` 与真机物理实测) 🏆

#### 1. 逆向突破背景与技术路线收敛
在早期的逆向工程中，针对“如何在 Even G2 对话模式中呈现上方带框的 AI 建议气泡”，曾存在两种推测：
- **推测 A**：双服务并发联动 —— 底部走 `0x0B-20`（对话），上方走 `0x05-20`（翻译）或 `0x0C-20`（Tasks）；
- **推测 B**：单服务原生支持 —— `Service 0x0B-20` 内部已经具备双视口渲染指令。

通过用户现场捕获的官方 App 真实蓝牙日志 [`tests/对话模式_有AI提示.pklg`](file:///Users/l.ylive.cn/OneDrive/smart-glass/tests/对话模式_有AI提示.pklg)（180 KB）以及硬件物理实测，**推测 B 得到 100% 证实**：
> **重大结论**：Even G2 官方在单一服务 **`Service 0x0B-20`** 内部即原生支持“**上半区 AI 胶囊卡片 + 下半区三行实时转写**”的一体化分屏渲染！

---

#### 2. AI 胶囊卡片协议报文深入解密 (`Type 5`)

官方 App 触发 AI 建议时下发的报文特征如下：

```protobuf
// ConversatePacket 主报文
type = 5;                                  // 核心命令类型 5: 上半区 AI 胶囊卡片
msg_id = 70;                               // 单调递增消息序号 (必须与外层 seq 对齐)
AIPromptPayload prompt = 7;                // Tag 7 (Field 7, Wire Tag 0x3A)
```

##### 内部载荷 `AIPromptPayload` (Wire Format 与实测字段规范):
| Tag | 字段名称 | Wire Type | 类型 | 实际取值 / 示例 | 硬件与 UI 渲染语义 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **1** | `card_type` | Varint (`0x08`) | `uint32` | **`4`** | **官方标准卡片组件类型**：激活固件底层标准圆角边框胶囊组件（头部附带智能 AI Emoji/图标）。<br>*(注：经对照交叉实测，卡片的常驻显示是由 UI 展开状态机驱动，推荐统一采用官方基准值 `4`)*。 |
| **2** | `title` | Length-delimited (`0x12`) | `string` | `"建议一：探讨核心架构"` | **胶囊主标题 (渲染锚点)**：**强制单行渲染**（容量约 12~14 汉字）。若未超长则完整显示无省略号；若超长或包含 `\n`，固件自动单行截断并在末尾补省略号 `...`。<br>🚨 **重要规则**：`title` 是卡片渲染的必要前提，若留空则整个胶囊显示空白。 |
| **3** | `detail` | Length-delimited (`0x1A`) | `string` | 详细长文本（支持多行展开） | **展开长文详细解读**：折叠时隐藏；**佩戴者单击镜腿触控板（Tap Touchpad）后立即向下撑开胶囊，显示多行完整内容**！ |
| **4** | `status` | Varint (`0x20`) | `uint32` | `0` | 状态标记位 (默认设为 0)。 |

##### 抓包原始分包实录 (`tests/对话模式_有AI提示.pklg` Packet #69):
官方推送包含详细解读的长提示时，报文长达 244 字节，固件协议栈自动采用标准双包分包机制：
- **分包 1** (`seq=0x45, pkt_tot=2, pkt_idx=1`，长度 232 字节)：下发前半部分 Protobuf 编码流；
- **分包 2** (`seq=0x45, pkt_tot=2, pkt_idx=2`，长度 18 字节)：下发剩余载荷，并在 Payload 紧邻末尾追加 **2 字节 CCITT CRC-16 校验码**（实测抓包值为 `[1A 42]`）。

---

#### 3. 物理真机实测呈现与多行容纳能力实测 (2026-09-12 真机闭环验证) 🏆

运行专用双视口验证工具 [`scripts/test_conversate_ai_capsule.py`](file:///Users/l.ylive.cn/OneDrive/smart-glass/scripts/test_conversate_ai_capsule.py) 与参数探测脚本 [`scripts/probe_capsule_expanded.py`](file:///Users/l.ylive.cn/OneDrive/smart-glass/scripts/probe_capsule_expanded.py)，在真实 Even G2 镜片上观测并证实了以下关键渲染与交互机制：

```text
未展开预览态 (Collapsed Capsule):
+-------------------------------------------------------------+
|         ╭─────────────────────────────────────╮             |
|         │ 🤖 建议一：探讨系统核心架构...          │   <--- 单行紧凑圆角胶囊 (超长自动截断加 ...)
|         ╰─────────────────────────────────────╯             |
+-------------------------------------------------------------+
                            ↓  佩戴者单击右镜腿 (Tap Touchpad)
单击展开详情态 (Expanded Card):
+-------------------------------------------------------------+
|         ╭─────────────────────────────────────╮             |
|         │ 🤖 建议一：探讨系统核心架构             │   <--- 展开态：显示完整标题
|         │ ----------------------------------- │             |
|         │ 第一行详情：系统架构完整打通。          │   <--- 垂直撑开容纳 detail 多行内容！
|         │ 第二行详情：显存与时序完全掌握。        │             |
|         ╰─────────────────────────────────────╯             |
+-------------------------------------------------------------+
```
...
##### 核心物理实测结论汇总：
1. **多行容纳能力**：
   - **胶囊折叠态**：固件**严格限制为单行**。单行容量约 12~14 个汉字。超长文本或显式 `\n` 均不会将外框撑高为两行，而是在单行末尾以省略号 `...` 截断；
   - **胶囊展开态**：佩戴者**单击镜腿后，胶囊自适应向下垂直撑开，完整容纳 `detail` 字段的 2~3 行甚至更多长文本**！
2. **`card_type` 的显示模式分流**：
   - **`card_type = 4`（临时 Toast 模态）**：弹出显示 4~6 秒后平滑淡出，不阻挡现实视野，归入后台历史卡片供触控调出；
   - **`card_type = 1` 与 `3`（常驻 Sticky 模态）**：弹出后持续常显，佩戴者单击镜腿展开详情后**依然保持持续显示，直到退出对话模式**！非常适合在演讲、会议提词或连续 Copilot 场景下常驻提示关键核心结论。
3. **“强制展开”的技术边界与 AR 安全设计原则**：
   - **固件本地交互接管**：单击镜腿展开胶囊是由眼镜端 MCU 本地硬件直接处理的（蓝牙通道无上行数据中断），零延迟瞬时展开；
   - **视野安全保护（Visual Safety Protection）**：AR 眼镜的人机工程规范严格限制后台应用擅自全屏弹出多行大卡片阻挡佩戴者现实视野（防止走路或会议时造成视野盲区）。因此固件设计为：**手机只负责下发折叠建议（Title）与详情（Detail），而决定何时展开多行大卡片的控制权完全交由佩戴者通过镜腿单击触发**。
   - **工程最佳实践**：
     - 若需佩戴者一目了然：将最关键的结论浓缩在 12 字以内的 `title`（如 `"结论：本季度营收增长23%"`），在胶囊内完整无省略号显示；
     - 辅助详细论据置于 `detail`，佩戴者需要时随时敲击镜腿展开阅读。

---

#### 4. 双视口一体化构造函数参考 (Python / iOS 网关工程化)

```python
def build_conversate_ai_prompt(seq: int, title: str, detail: str = "", card_type: int = 4) -> bytes:
    """
    构造 0x0B-20 对话模式下的原生 AI 胶囊提示卡片 (Type 5)
    
    :param seq: 全局单调递增包序号
    :param title: 胶囊卡片主标题 (显示在圆角线框内)
    :param detail: 展开详细解读文本 (触控板交互展开)
    :param card_type: 卡片类型，默认为 4 (标准圆角胶囊边框 + 智能图标)
    """
    # 1. 构造内部 AIPromptPayload
    prompt_payload = bytearray()
    prompt_payload.extend(encode_varint(1, card_type))       # Tag 1: card_type = 4
    prompt_payload.extend(encode_string(2, title))           # Tag 2: title
    if detail:
        prompt_payload.extend(encode_string(3, detail))       # Tag 3: detail
    prompt_payload.extend(encode_varint(4, 0))               # Tag 4: status = 0
    
    # 2. 构造外层 ConversatePacket
    packet_body = bytearray()
    packet_body.extend(encode_varint(1, 5))                  # Tag 1: type = 5 (AI Prompt)
    packet_body.extend(encode_varint(2, seq))                # Tag 2: msg_id = seq
    packet_body.extend(encode_length_delimited(7, prompt_payload)) # Tag 7: prompt (0x3A)
    
    # 3. 封装 G2 协议帧头 (Service 0x0B-20)
    return wrap_g2_packet(seq=seq, svc_hi=0x0B, svc_lo=0x20, payload=packet_body)
```

---

#### 5. 长文本切片流式走马灯与“卡片抽屉列表栈 (Card Stack)”机制 (2026-09-12 物理实测重大发现) 🏆

针对单行胶囊 12~14 字符容量限制，通过多轮真实硬件实测，确立了将长文本拆分为多段短文本的**时间切片流（Temporal Slicing Stream）与卡片栈（Card Stack）体系**：

```text
1. 前台免触碰走马灯放映 (0s -> 3s -> 6s 同一视口槽位连续刷新):
+-------------------------------------------------------------+
|         ╭─────────────────────────────────────╮             |
|   0.0s: │ 🤖 要点一：核心架构已验证             │             |
|   3.0s: │ 🤖 要点二：显存时延为2.25秒          │   <--- 免触碰连贯阅读几十上百字！
|   6.0s: │ 🤖 要点三：成功呈现多段长文          │             |
|         ╰─────────────────────────────────────╯             |
+-------------------------------------------------------------+
                                ↓  佩戴者单击右镜腿 (Tap Touchpad)
2. 呼出卡片抽屉列表栈 (Card Stack / List Mode):
+-------------------------------------------------------------+
|         ╭─────────────────────────────────────╮             |
|  Top 1: │ 🤖 [Card 3] 要点三：成功呈现多段长文   │             |
|         │ ----------------------------------- │             |
|         │ 详情: 通过流水轮播实现长文连贯阅读。   │   <--- 每个胶囊均可独立展开 detail！
|         ╰─────────────────────────────────────╯             |
|         ╭─────────────────────────────────────╮             |
|  Top 2: │ 🤖 [Card 2] 要点二：显存时延为2.25秒 │             |
|         ╰─────────────────────────────────────╯             |
|         ╭─────────────────────────────────────╮             |
|  Top 3: │ 🤖 [Card 1] 要点一：核心架构已验证   │             |
|         ╰─────────────────────────────────────╯             |
+-------------------------------------------------------------+
```
...
##### 核心系统机制与人机交互（HCI）规约：
1. **前台视觉（同一槽位流式走马灯）**：
   - 固件在屏幕上方同一位置（Slot）依次替换渲染传入的切片胶囊；
   - 佩戴者**无需任何物理触控**，视线保持在上方同一区域，即可像阅读走马灯提词器一样，在十几秒内流畅阅览长达几十至上百字的复杂论据或多条要点；
2. **后台历史栈（卡片抽屉列表）**：
   - 所有流式下发过的切片胶囊**均不会被销毁**，而是被固件按时间倒序压入会话卡片栈（Card Stack）；
   - 佩戴者**单击右镜腿（Tap）**，固件瞬间拉出**多卡片纵向排列列表**，最新的胶囊显示在最顶部；
   - 🌟 **全功能可展开**：列表中的**每一个胶囊卡片都保持完整的交互能力，均可独立展开查看各自的 `detail` 详细长文**！
3. **卡片栈容量极限与 FIFO 淘汰机制 (2026-09-12 100次终极压测证实) 🏆**：
   - **吞吐与流控稳定性**：以 0.25 秒/张的高速节拍连续注入 **100 张** 独立胶囊卡片，前台单一视口依次平滑放映了全部 100 张卡片，**眼镜协议栈与渲染引擎 100% 承接，零丢帧、零丢包、零假死**；
   - **卡片栈绝对物理容量**：**精确测定为 20 条（20 Items Ring Buffer）**！
     - 注入 100 张后翻阅抽屉列表，最顶部为 `#100`，最底部最老卡片**严格截断在 `#081` 号**（$100 - 81 + 1 = 20$）；
     - 固件在卡片栈上实现了标准的 **FIFO 环形队列（First-In-First-Out）**：当卡片超过 20 条时，自动将最早的历史卡片安全出栈淘汰，永远为佩戴者保留**最新的 20 条高价值 AI 提词卡片**；
   - **交互展开生命周期机制**：
     - **展开态（Expanded）**：佩戴者单击镜腿展开详情后，**卡片处于持续常驻显示状态（Persistent）**，绝不限时自动退场，供佩戴者从容阅读长文推导；
     - **收拢态（Collapsed）**：再次单击镜腿收拢回单行胶囊后，**停留数秒即自动平滑淡出隐藏**，视野迅速恢复自然透视状态；
   - **产品工程指导线**：单次会话无需担心卡片溢出崩溃，网关可放心大批量推送要点，固件自带 20 槽位滚动保护窗，完全胜任长达 1~2 小时的高频会议/课堂智能伴读。
4. **与大模型（LLM）流式输出的工业级天作之合**：
   - 大模型在云端生成一段包含多项结论的长文本时，网关利用正则或语义切分器（Sentence / Bullet-Point Splitter）将回复拆分为多个 10~12 字的核心要点并提取对应解释；
   - 网关以 2.5 ~ 3.0 秒的时钟间隔按序向眼镜推送切片胶囊；
   - **佩戴者体验达到极致**：
     - **即时态**：抬眼免触碰看完大模型的多点摘要轮播；
     - **沉淀态**：敲一下镜腿，完整的多点提纲清单与详细长文全部静候在列表中供随时翻阅复习！

---

#### 6. 展开提示事件上报全协议解密与主动发包展开可行性分析 (基于 `tests/对话模式_有AI提示_有展开提示操作.pklg` 捕获实证) 🆕

通过对用户现场触发“AI提示并单击镜腿展开”的完整抓包文件 [`tests/对话模式_有AI提示_有展开提示操作.pklg`](file:///Users/l.ylive.cn/OneDrive/smart-glass/tests/对话模式_有AI提示_有展开提示操作.pklg)（184 KB）进行高精度数据流剖析，并与历史抓包进行三向对比，彻底解密了“展开提示”时的底层蓝牙数据行为：

##### 1. 三向抓包严密对照分析
| 抓包文件 | 是否包含展开操作 | `Service 0x0B-01` 上报帧次数 | 报文特征与捕获状态 |
| :--- | :--- | :--- | :--- |
| `tests/对话模式_无AI提示.pklg` | ❌ 无 | **0 次** | 无任何事件上报 |
| `tests/对话模式_有AI提示.pklg` | ❌ 未展开 | **0 次** | 仅有常规转写与下发 ACK |
| **`tests/对话模式_有AI提示_有展开提示操作.pklg`** | ✅ **有展开操作** | **恰好 1 次！** | **捕获关键帧 `0x0B-01 (Type 3 响应)`** |

> 🌟 **100% 因果确证**：`Service 0x0B-01` 报文是固件在佩戴者执行“展开卡片”物理手势时才会产生的**唯一专属事件上报帧**！

##### 2. 展开上报帧逐字节解密 (Wire Format & Raw Hex)
```text
Offset 0x02432E (Packet #69):
AA 12 2E 0C 01 01 0B 01 08 A3 01 10 4C 62 03 20 9A 2D [CRC 89 CD]
```
- **帧头**：`AA 12`（RX 标识，由眼镜发往手机）；
- **包序号**：`seq = 0x2E`；
- **服务通道**：`Service 0x0B-01`（G2 对话服务事件上报通道）；
- **Protobuf 载荷解码**：
  - `08 A3 01`：**`Tag 1 = 163 (0xA3)`**。根据 G2 协议架构规律：`上行响应码 = 控制命令枚举 + 160 (0xA0)`。
    $$163 - 160 = \mathbf{3}$$
    **确凿证实：在固件状态机中，“卡片展开交互（Expand Card）”对应的底层核心命令枚举即为 `Type = 3`！**
  - `10 4C`：`Tag 2 = 76 (0x4C)`，对齐手机当前的 `msg_id`；
  - `62 03 20 9A 2D`：`Tag 12` 长度 3 字节，内部为 Varint `Tag 4 = 5786`（展开卡片的元数据索引/状态标）。

##### 3. 官方 App 交互机制真机定性
- **眼镜端本地接管**：佩戴者单击镜腿展开胶囊时，展开动画与 `detail` 多行渲染是由眼镜端 MCU **直接在本地硬件闭环完成的**，手机端在展开瞬间并没有下发任何控制报文；
- **异步状态通知**：展开完成后，眼镜通过 `0x0B-01` 异步通知手机：“佩戴者刚才展开了卡片（Type 3 事件）”，手机 App 接收后仅需按常规下发 `Type 255 (Flush Sync)` 维持心跳。

##### 4. 主动发包展开真机实测结论 (2026-09-12 否定性实测验证) 🏆
针对“能否通过主动下发 `0x0B-20 Type 3` 报文强制眼镜展开卡片”，通过编写测试工具 [`scripts/test_active_expand_capsule.py`](file:///Users/l.ylive.cn/OneDrive/smart-glass/scripts/test_active_expand_capsule.py) 在真机上进行了对照下发实测：
- **物理实测现象**：手机下发 `Type 5` 胶囊后，再主动向 `0x0B-20` 下发 `Type 3` 展开报文，**眼镜依然保持带省略号的单行折叠胶囊，未被触发自动展开**；
- **底层架构定性**：
  1. **单向状态通知机制（Notification-Only）**：`Service 0x0B-01 (Tag 1 = 163)` 是眼镜在佩戴者执行物理单击展开后，单向通知手机的**状态广播事件**，而非双向 RPC 调用；
  2. **硬件独占特权（Hardware-Exclusive Privilege）**：固件窗口管理器将“多行大卡片展开”严格限定为**镜腿物理触控板硬件中断专有**，底层未开放远程软件强行展开接口，以在系统级别绝对杜绝第三方应用擅自全屏遮挡佩戴者自然视野的安全隐患；
- **自研应用工程收敛指南**：
  - **无需纠结软件远程展开**：长文本的多行连贯阅读**完全由我们在 §29.8.5 中实证的“时间切片流式走马灯（Carousel）”完美解决**（免触碰依次放映全部要点，同时自动在后台沉淀为 20 条镜腿可展开列表）；
  - **手势联动**：网关可通过监听 `0x0B-01` 报文感知佩戴者何时展开了哪一张卡片，实现手机端 UI 与眼镜端佩戴者注意力的毫秒级精准同步！

---

## 30. Even G2 独立翻译模式 (Service 0x05-20 Live Translation) 全协议解密与顶部逐句对照视口规范 (基于 2026-09 翻译模式_中译英.pklg 实测) 🆕

### 30.1 重大逆向发现与业务定位区分
在过去开源社区的逆向工程中，普遍误以为“对话”与“翻译”走的是同一个 `0x0B-20` 服务。然而通过用户现场抓取的 `tests/翻译模式_中译英.pklg`（224 KB，含 90 个 0xAA 报文）深入剖析，证实了**翻译模式（Translate Mode）是一个底层完全独立的专用服务通道：`Service 0x05-20`**！

#### 对话模式 (`0x0B-20`) 与 翻译模式 (`0x05-20`) 的核心本质区别：
| 维度 | **对话模式 (`Service 0x0B-20`)** | **翻译模式 (`Service 0x05-20`)** |
| :--- | :--- | :--- |
| **底层 Service ID** | **`0x0B-20`** (ACK: `0x0B-00`) | **`0x05-20`** (ACK: `0x05-00`) |
| **物理视口位置** | **底部 3 行吸底视口 (Bottom-Anchored)** | **顶部视口 (Top-Anchored)** |
| **内容呈现机制** | **流式增量打字与滚屏** (`is_final=0` 逐字更新，`1` 向上滚一行) | **整句双语对照呈现 (Sentence-by-Sentence)** (原文 + 译文整句同时推送) |
| **说话人标记** | 隐式状态位 | 显式 UTF-16BE 字段：`Speaker 1` / `Speaker 2` |
| **分包传输支持** | 单包流式 | 超过 MTU 时采用标准分包机制 `(1/2), (2/2)` / `(1/3), (2/3), (3/3)` |

---

### 30.2 顶部逐句视口物理机制
现场物理观察表明：“翻译是逐句进行的，文本显示在顶部几行”。
底层机理与排版机制解析如下：
1. **顶部锚定（Top-Anchored Viewport）**：
   - 固件为 `0x05-20` 分配的可视区域位于 MicroLED 光学引擎的**上半部/顶端几行**；
   - 这种排版针对“单向精读翻译”优化——佩戴者目光只需微抬即可完整阅读整句译文，而下半区保持干净透光；
2. **整句置换而非连续滚屏**：
   - 翻译模式下，算法在手机端完成 VAD 整句断句与神经翻译后，以一个完整的 `Type 2` 复合报文下发；
   - 报文同时携带**原文（Original）**与**译文（Target）**，在顶部视口形成固定的上下文卡片，下一句到来时整体刷新或翻页，不产生逐字滚动的视觉干扰。

---

### 30.3 Protobuf 全量 Schema 规格定义 (`Service 0x05-20`)

```protobuf
syntax = "proto3";
package even.g2;

// Service 0x05-20 翻译模式主结构体
message TranslationPacket {
  uint32 type = 1;                        // 1 = 会话控制, 2 = 逐句双语对照更新, 255 = 提交同步标杆
  uint32 msg_id = 2;                     // 单调递增消息 ID
  
  // 当 type = 1 时有效 (会话启停)
  TranslationSessionControl control = 3; // Tag 3 (Field 3)
  
  // 当 type = 2 时有效 (逐句双语翻译核心负载)
  TranslationSentence sentence = 4;      // Tag 4 (Field 4)
  
  // 当 type = 255 时有效 (提交/心跳标记)
  TranslationMarker marker = 8;          // Tag 8 (Field 8, 空内容 42 00)
}

message TranslationSessionControl {
  uint32 action = 1;                     // 1 = 启动翻译会话, 2 = 停止退出会话
  string language_pair = 2;              // 语言对标识符, 如 "ZH>EN", "EN>ZH", "AUTO>ZH"
  uint32 status = 3;                     // 状态标志
  repeated LanguageItem support_languages = 5; // 支持的语言列表
  string default_target = 6;             // 默认目标语言 (如 "EN")
}

message TranslationSentence {
  string original_text = 1;              // 识别出的原文完整句子 (UTF-8)
  string translated_text = 2;            // 机器翻译后的译文完整句子 (UTF-8)
  uint32 flag1 = 3;                      // 标志位 (通常为 0)
  uint32 flag2 = 4;                      // 标志位 (通常为 0)
  bytes speaker_name = 5;                // 说话人名称标识, 格式为带 BOM 的 UTF-16BE: "Speaker 1"
}

message TranslationMarker {
  // 空负载 (42 00)，通知光机锁存当前句
}
```
...
### 30.4 真实抓包十六进制数据剖析 (Raw Hex & Wire Format)

#### 1. 翻译模式启动会话报文 (`Type 1 Action 1`, 语言对 `ZH>EN`, 长度 210B)
```text
AA 21 2A D2 01 01 05 20 08 01 10 2A 1A C9 01 08 01 12 05 5A 48 3E 45 4E 18 01 2A 14 ... [CRC16]
```
- `08 01`：`type = 1`；
- `10 2A`：`msg_id = 42`；
- `1A C9 01`：`Field 3` 长度 201 字节；
  - `08 01`：`action = 1` (启动)；
  - `12 05 5A 48 3E 45 4E`：`Tag 2` 字符串为 **`"ZH>EN"`**（中文译英文模式）；
  - `18 01`：`Tag 3 = 1`。

#### 2. 逐句双语对照更新报文 (`Type 2`, 长度 232B, 分包 1/2)
```text
AA 21 3C E8 02 01 05 20 08 02 10 3C 22 C9 02 0A A0 01 31 39 38 34 E6 98 AF E8 8B B1 ... [CRC16]
```
- `AA 21 3C E8 02 01`：长度 232 字节，`tot=2, idx=1`（第 1 个分包）；
- `08 02`：`type = 2`（逐句翻译更新）；
- `10 3C`：`msg_id = 60`；
- `22 C9 02`：`Field 4` 长度 329 字节复合结构：
  - `0A A0 01 ...`：`Tag 1` 原文：“`1984是英国主义作家乔治·奥威尔于1949年出版的长篇政治小说，在这部作品中奥威尔刻画了一个令人感到窒息的恐怖世界。`”；
  - `12 ...`：`Tag 2` 译文：“`1984 is a long political novel by British author George Orwell published in 1949, in which Orwell portrays a suffocating world of terror.`”；
  - `2A 14 FE FF 00 53 00 70 00 65 00 61 00 6B 00 65 00 72 00 20 00 31`：`Tag 5` 带有 UTF-16BE 字节序标记的说话人：**`"Speaker 1"`**。

#### 3. 退出翻译模式注销报文 (`Type 1 Action 2`, 长度 12B)
```text
AA 21 46 0C 01 01 05 20 08 01 10 46 1A 04 08 02 20 00 [E9 F9]
```
- `08 01`：`type = 1`；
- `10 46`：`msg_id = 70`；
- `1A 04`：`Field 3` 长度 4 字节：
  - `08 02`：`action = 2` (**退出/注销会话**)；
  - `20 00`：`status = 0`；
- 眼镜端在 `0x05-00` 响应 `0x12 ... 08 A2 01 10 46 3A 00` ACK，视口安全注销。

---

### 30.5 架构启示：G2 屏幕纵向空间的双模物理归属
通过对比这两天的三次抓包，Even G2 的 MicroLED 物理屏幕布局之谜彻底解开：
```text
┌────────────────────────────────────────────────────────┐
│ 【顶部视口 (Top Region)】                               │
│  • 专供 Service 0x05-20 (翻译模式: 原文+译文整句对照)   │  <-- 顶部几行
│  • 或 自研系统承载 AI Copilot 提示卡片                  │
├────────────────────────────────────────────────────────┤
│ 【底部视口 (Bottom Region)】                            │
│  • 专供 Service 0x0B-20 (对话模式: 实时流式转写)        │  <-- 底部三行
│  • 随说随出，is_final 平滑上滚                          │
└────────────────────────────────────────────────────────┘
```
这一发现完美证实了：**G2 固件在原生设计上就天然将顶部空间与底部空间赋予了不同的服务通道**！自研网关既可以独立复现 `0x05-20` 的精美双语对照，也可以在 `0x0B-20` 底部转写的同时，在上半部叠加我们的智能话题提示！
