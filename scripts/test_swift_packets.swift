...
var initPayload = Data([0x08, 0x01, 0x10])
initPayload.append(encodeVarint(initMsgId))
initPayload.append(Data([0x1A]))
initPayload.append(encodeVarint(settings.count))
initPayload.append(settings)

let initPkt = buildPacket(seq: &initSeq, serviceHi: 0x06, serviceLo: 0x20, payload: initPayload)
print("Swift Init:")
print(initPkt.map { String(format: "%02x", $0) }.joined())

let testPayload = Data([
    0x08, 0x01, 0x10, 0x15, 0x1a, 0x1d, 0x08, 0x01, 0x12, 0x19, 0x08, 0x00, 
    0x10, 0x00, 0x18, 0x00, 0x20, 0x3b, 0x28, 0xc9, 0x04, 0x30, 0xb7, 0x04, 
    0x38, 0xa9, 0x18, 0x40, 0x00, 0x48, 0x01, 0x50, 0x09, 0x58, 0x00
])
let crcSwift = crc16CCITT(testPayload)
print(String(format: "Swift CRC: %04x", crcSwift))
