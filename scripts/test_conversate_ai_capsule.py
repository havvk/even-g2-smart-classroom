#!/usr/bin/env python3
"""
Even G2 对话模式 (0x0B-20) 官方原生 AI 胶囊提示 + 底部转写 双视口一体化实测工具

基于 2026-09-12 抓包《对话模式_有AI提示.pklg》100% 还原:
- Service: 0x0B-20
- 底部转写: Type 6 (Field 8), is_final 控制流式与滚行
- 顶部胶囊: Type 5 (Field 7), card_type=4 (圆角胶囊边框 + Emoji + 标题 + 详细解释)
- 显存锁存: Type 255 (Field 11, 5A 00)
"""
...
def build_conversate_ai_prompt(seq: int, title: str, detail: str = "", card_type: int = 4) -> bytes:
    """
    构造官方原生的 0x0B-20 Type 5 (Field 7) AI 胶囊提示报文
    - card_type: 4 (标准圆角胶囊边框 + 智能图标)
    - title: 胶囊内展示的短标题
    - detail: 展开卡片展示的详细解读 (可通过手势交互查看)
    """
    ...
    # 构造 Field 7 内部子消息
    sub = bytearray()
    # Tag 1 (0x08): card_type (varint)
    sub += bytes([0x08, card_type & 0x7F])
    # Tag 2 (0x12): title (string)
    sub += bytes([0x12]) + encode_varint(len(title_b)) + title_b
    if detail_b:
        # Tag 3 (0x1A): detail (string)
        sub += bytes([0x1A]) + encode_varint(len(detail_b)) + detail_b
    # Tag 4 (0x20): status = 0
    sub += bytes([0x20, 0x00])
    
    # 构造 0x0B-20 顶层 Protobuf 消息
    # Tag 1 (0x08): type = 5
    # Tag 2 (0x10): msg_id = seq
    # Tag 7 (0x3A): prompt = sub
...
async def main():
    print("=" * 70)
    print("🚀 Even G2 原生 AI 胶囊提示 + 底部转写 双视口一体化实测")
    print("=" * 70)
    ...
    if g2_devices:
        ...
        print(f"⚡ 使用预设已知设备地址连接: {target_addr}\n")
    ...
    async with BleakClient(target_addr) as client:
        ...
        # 3. 底部下发第一句实时转写
        s_t1 = next_seq()
        trans1 = "【底部转写】英国作家乔治奥威尔的代表作是什么？"
        print(f"👉 [步骤 2] 下发底部语音转写 (seq={s_t1}): '{trans1}'")
        ...
        # 4. 重点来了！向上半区注入官方原生 Type 5 AI 胶囊提示卡片！
        s_ai = next_seq()
        capsule_title = "奥威尔1949年出版的作品是《1984》"
        capsule_detail = "乔治·奥威尔于1949年出版的长篇政治小说是《1984》，提出了“老大哥”等经典概念。"
        print(f"\n🌟 [步骤 3: 核心高光] 向上半区注入原生 AI 胶囊卡片 (seq={s_ai}):")
        print(f"   胶囊标题: '{capsule_title}'")
        print(f"   展开详情: '{capsule_detail}'")
        ai_pkt = build_conversate_ai_prompt(s_ai, title=capsule_title, detail=capsule_detail, card_type=4)
        ...
        # 5. 底部继续追加第二句转写（验证上下半区并行无干扰）
        s_t2 = next_seq()
        trans2 = "【底部追加】在这部经典作品中，刻画了怎样的社会？"
        print(f"👉 [步骤 4] 底部继续追加第二句转写 (seq={s_t2}): '{trans2}'")
        await client.write_gatt_char(CHAR_WRITE, build_conversate_transcript(s_t2, trans2, is_final=True), response=False)
        s_sync3 = next_seq()
        await client.write_gatt_char(CHAR_WRITE, build_conversate_sync(s_sync3), response=False)

        print("\n" + "=" * 60)
        print("👀 【请仔细观察镜片全局】:")
        print(f"   1. 上半部分：是否看到了带圆角边框的胶囊卡片？标题为 '{capsule_title}'？")
        print(f"   2. 下半部分：是否同时滚动显示着底部两行语音转写？")
        print("   3. 您可以尝试用手指轻触或轻滑右镜腿触摸板，看胶囊是否有互动反应。")
        print("⏳ 保持屏幕常显 15 秒供您充分观察与体验...")
        print("=" * 60)
