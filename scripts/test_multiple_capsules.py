#!/usr/bin/env python3
"""
Even G2 多胶囊堆叠与长文本分段流式展示实测工具

测试目标:
1. 并发堆叠探测: 瞬时连续下发 2 个不同内容的 Type 5 胶囊，观察固件是同时堆叠显示多个胶囊，还是后包覆写前包？
2. 流式走马灯探测: 将长文本切片为多段 10~12 字短文本，按 3 秒间隔依次流式推送到胶囊视口，测试连贯阅读体感。
"""
...
async def main():
    mode = sys.argv[1] if len(sys.argv) > 1 else "stack"  # "stack" 或 "stream"

    print("=" * 70)
    print(f"🧪 Even G2 多胶囊渲染机制探测实测 (Mode: {mode})")
    print("=" * 70)
    ...
    async with BleakClient(target_addr) as client:
        ...
        # 流式走马灯放映模式: 将长文本切片为 3 段，每段 10~12 字，间隔 3 秒依次更新放映
        slices = [
            ("要点一：核心架构已验证", "第一部分详情：双视口底层通信完全打通。"),
            ("要点二：显存时延为2.25秒", "第二部分详情：精准避让开场动画保护期。"),
            ("要点三：成功呈现多段长文", "第三部分详情：通过流水轮播实现长文连贯阅读。")
        ]
        
        print("🌟 [流式走马灯放映] 将长文本拆分为 3 个紧凑胶囊，每隔 3.0 秒流式刷新...")
        for idx, (t, d) in enumerate(slices):
            s_ai = next_seq()
            print(f"\n👉 [流水放映第 {idx+1}/3 段] seq={s_ai}: '{t}'")
            pkt = build_conversate_ai_prompt(s_ai, title=t, detail=d, card_type=4)
            await client.write_gatt_char(CHAR_WRITE, pkt, response=False)
